...
class EvalNetworkModel(keras.Model):

    def __init__(self):
        super(EvalNetworkModel,self).__init__(name='')
        self.drop_out = keras.layers.Dropout(rate=0.2)
        self.LReLU = LeakyReLU(alpha=LEARKYRELU_ALPHA)
        self.flatten = keras.layers.Flatten()
        self.conv2d_1 = keras.layers.Conv2D(filters=32, kernel_size=2, strides=(1,1), 
                        padding='valid', activation=self.LReLU)
        ...
        self.conv2d_8_eval = keras.layers.Conv2D(filters=2, kernel_size=1, strides=(1,1), 
                        padding='same', activation=self.LReLU)
    
        self.global_average_pool = keras.layers.GlobalAveragePooling2D()
        self.dense_9_eval = keras.layers.Dense(2, activation='softmax')
    ...
    def call(self, input_tensor):
        ...
        output_8_eval = self.global_average_pool(self.conv_drop(self.conv2d_8_eval, output_7))
        eval_output = self.dense_9_eval(self.flatten(output_8_eval))
        return eval_output

def create_model():
    return EvalNetworkModel()